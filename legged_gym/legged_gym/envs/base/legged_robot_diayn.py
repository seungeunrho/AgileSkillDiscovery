...
class LeggedRobotDiayn(LeggedRobotNoisy):
    ...
    def _init_buffers(self):
        ...
        self.skills_log_prob = torch.zeros(self.num_envs, self.skill_dim, device=self.device, dtype=torch.float, requires_grad=False)
        self.prev_skill_obs_buf = torch.zeros(self.num_envs, self.num_obs, device=self.device, dtype=torch.float, requires_grad=False)
        self.idx = torch.zeros(self.num_envs,device = self.device, dtype=torch.int64, requires_grad=False)
    ...
    def reset_idx(self, env_ids):
        ...
        if self.cfg.env.sample_skill:
            # dist=  torch.normal(0, 1, size=(len(env_ids), self.skill_dim)).to(self.device)
            # dist = torch.distributions.Normal(0,1)
            # import ipdb;ipdb.set_trace()
            # self.skills[env_ids] = torch.normal(0, 1, size=(len(env_ids), self.skill_dim)).to(self.device)
            if self.cfg.env.mode=="discrete":
                # import ipdb;ipdb.set_trace()
                self.idx[env_ids] = torch.randint(0,self.skill_dim,(len(env_ids),),device = self.device)
                # if len(env_ids)>0:
                self.skills[env_ids] = 0
                self.skills[env_ids,self.idx[env_ids]] = 1
                if torch.sum(self.skills)>self.num_envs:
                    print("skills wrong")

            elif self.cfg.env.mode=="continuous":
                dist = torch.distributions.Normal(0,1)
                self.skills[env_ids] = dist.sample((len(env_ids), self.skill_dim)).to(self.device)
                self.skills_log_prob[env_ids] = dist.log_prob(self.skills[env_ids]).to(self.device)
            
            
            # self.skills_log_prob[]
    ...
    def compute_observations(self):
        ...
        self.prev_skill_obs_buf[:] = self.obs_buf[:]
    ...
    def _reward_diversity(self):
        with torch.no_grad():
            ...
            q_s =  self.actor_critic.discriminator_inference(self.prev_skill_obs_buf)
            
            if self.cfg.env.mode=="discrete":
                # import ipdb;ipdb.set_trace()
                int_rew = torch.log(q_s[torch.arange(len(q_s)),self.idx]) - torch.log(torch.ones((self.num_envs),device=self.device)*1/self.skill_dim)
            elif self.cfg.env.mode=="continuous":
                int_rew = torch.log(q_s) - self.skills_log_prob
