...
class PPOMetraRNN(PPO):
    ...
    def compute_losses(self, minibatch):
        ...
        value_batch, div_value_batch = self.actor_critic.evaluate(minibatch.critic_obs, masks=minibatch.masks,
                                                 hidden_states=minibatch.hid_states[1], hidden_states_dc = minibatch.hid_states[2])
        # value_batch, div_value_batch = self.actor_critic.evaluate(minibatch.critic_obs, masks=minibatch.masks,
        ...
        if self.adjustable_kappa==False and self.kappa==0:
            # print("i work for no div")
            value_loss = value_loss
        else:
            value_loss = value_loss + div_value_loss
        # if self.actor_critic.is_recurrent:
        #     phi_next_obs = self.actor_critic.discriminator_inference(unpad_trajectories(minibatch.next_obs, minibatch.masks[1]).flatten(0,1))
        #     phi_obs = self.actor_critic.discriminator_inference(unpad_trajectories(minibatch.next_obs, minibatch.masks[0]).flatten(0,1))exit
        # print(minibatch.next_obs.shape)
        # if self.actor_critic.is_recurrent:
        #     phi_next_obs = self.actor_critic.discriminator_inference(minibatch.raw_next_obs)
        #     phi_obs = self.actor_critic.discriminator_inference(minibatch.raw_obs)
        #     z = minibatch.raw_obs[:, -self.actor_critic.skill_dim:]
        # else:
        if self.actor_critic.is_recurrent:
            phi_next_obs = self.actor_critic.discriminator_inference(minibatch.next_obs.flatten(0,1))
            phi_obs = self.actor_critic.discriminator_inference(minibatch.obs.flatten(0,1))
            z = minibatch.obs.flatten(0,1)[:, -self.actor_critic.skill_dim:]
            ...
            lamda_loss = self.actor_critic.lamda * torch.clamp(1. - phi_norm_squared.detach(),
                                                            max=self.actor_critic.epsilon)
            phi_loss = -(phi_loss * (minibatch.masks.flatten(0,1))).mean()
            lamda_loss = (lamda_loss * (minibatch.masks.flatten(0,1))).mean()
        else:
